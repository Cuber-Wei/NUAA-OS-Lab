             ...
             struct fuse_file_info *info)
{
    ...
    inode_t *leaf = (inode_t *)(intptr_t)(info->fh);
    int count = inode_read(leaf, offset, buf, size);
    return count;
}
              ...
              struct fuse_file_info *info)
{
    ...
    inode_t *leaf = (inode_t *)(intptr_t)(info->fh);
    int count = inode_write(leaf, offset, (char *)buf, size);
    return count;
}
                  ...
                  off_t offset, struct fuse_file_info *info)
{
    ...
    while ((entry_ptr = dir_cursor_next(&dir_cursor)) != NULL) {
        fill(buf, entry_ptr->name, NULL, 0);
    }
